...
class ControllerUrgentCargusIstoric extends Controller {
    ...
    public function index(){
        ...
        $this->load->language('urgentcargus/istoric');
        ...
        $data['urgentcargus_preferinte_pickup']  = $pickup;
        $data['token'] = $this->session->data['token'];
        $data['view_url'] = $this->url->link('urgentcargus/istoric', 'token=' . $this->session->data['token'], 'SSL');
        ...
        if (is_array($token)) {
            ...
            if (isset($_GET['BarCode'])) {
                ...
                // UC get detalii awb
                $data['detaliuAwb'] = $this->model_shipping_urgentcargusclass->CallMethod('Awbs?&barCode=' . $this->request->get['BarCode'], array(), 'GET', $token);
                ...
                $data['awbs'] = $this->model_shipping_urgentcargusclass->CallMethod('Awbs?&orderId=' . $this->request->get['OrderId'], array(), 'GET', $token);
            } else {
                $data['zone'] = 'orders';
                ...
                $data['orders'] = $this->model_shipping_urgentcargusclass->CallMethod('Orders?locationId='.$pickup.'&status=1&pageNumber=1&itemsPerPage=100', array(), 'GET', $token);
            }
        }
        ...
        $this->response->setOutput($this->load->view('urgentcargus/istoric.tpl', $data));
    }
}
?>