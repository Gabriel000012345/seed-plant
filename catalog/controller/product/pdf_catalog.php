...
class ControllerProductPdfcatalog extends Controller
{
    ...
    public function createPdf($pdf_data)
    {
        ...
        if ($datacount > 1) {
            ...
            $pdf_logo = "../../../../image/tcpdf_logo.jpg";

            if (file_exists($pdf_logo) == false) {
                $pdf_logo = "tcpdf_logo.jpg";
            }
...
//                        $thumb = str_replace(HTTP_SERVER, "", $thumb);
                        $tmp_product = str_replace("{::product_cnt}", ($no_of_item+1), $html_template_product);
                        $tmp_product = str_replace("{::txt_product_name}", $this->language->get('text_new_product') . ':', $tmp_product);
                        $tmp_product = str_replace("{::product_name}", $product['name'], $tmp_product);
                        $tmp_product = str_replace("{::txt_product_subst}", "Subst. activa:", $tmp_product);
                        $product['upc'] = $product['upc'] == '' ? '--' : $product['upc'];
                        $tmp_product = str_replace("{::product_subst}", $product['upc'], $tmp_product);
...
//                            var_export($product['options']); die;
                            $options = $product['options'];
                            $product_options = '<table cellpadding="3px" cellspacing="0" border="1">';
                            $poc = count($options);
//                            $poc = min($max_options, $poc);

                            for ($k = 0; $k < $poc; $k++) {
                                if (isset($options[$k]['product_option_value']) && is_array($options[$k]['product_option_value'])) {
                                    $peroc = count($options[$k]['product_option_value']);
//                                    $peroc = min($max_per_options, $peroc);
                                    for ($i = 0; $i < $peroc; $i++) {
//                                        var_export($options[$k]['product_option_value'][$i]);
                                        $product_options .= '<tr><td>' . $options[$k]['product_option_value'][$i]['name'];
                                        if ($options[$k]['product_option_value'][$i]['price'] != "0") {
                                            $product_options .= '</td><td>' . $options[$k]['product_option_value'][$i]['price_prefix'] . ' '
                                                . $this->currency->format($this->tax->calculate(
                                                    $options[$k]['product_option_value'][$i]['price'],
                                                    $product['tax_class_id'],
                                                    $this->config->get('config_tax')
                                                ), $this->session->data['currency']);
                                        }
                                        $product_options .= '</td></tr>';
                                    }
                                }
...
//                            $tmp_product = str_replace("{::txt_product_options}", $this->language->get('text_product_options'), $tmp_product);
                            ...
                            $tmp_product = str_replace("{::product_options}", '<p style="text-align: center">' . $product['price'] .'</p>', $tmp_product);
